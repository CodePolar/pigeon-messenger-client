...
const CreateRoom = ({ chats }) => {
  ...
  const { refresh_rooms, setFocus, contacts, setContacts } =
    ...
    onSubmit: (values) => {
      ...
      createRoom((res) => {
        if (members.length > 0) {
          setAlert({
            type: "info",
            text: `Request to join / room sended`,
            resalt: `${values.title}`,
            show: true,
          });
        }
  ...
  const addMember = (e, i) => {
    let index = members.findIndex(
      (x) => x.contact_id.username === e.contact_id.username
    );
    ...
    if (index < 0) {
      ...
      let deleteMember = members.filter(
        (r) => r.contact_id.username !== e.contact_id.username
      );
      setContacts(contactFiltered);
  ...
  const createRoom = async (cb) => {
    ...
    formData.append(
      "document",
      JSON.stringify({ name: title, members: user, admin: user, description })
    );
    ...
    if (members.length > 0) {
      for (let member of members) {
        request_room({
          img: res.data.room.img,
          requester: user.username,
          user_id: member.contact_id._id,
          room_id: res.data.room.room_id,
          title: res.data.room.name,
          description: res.data.room.description,
          username: member.contact_id.username,
        });
      }
    }
    ...
    <>
      ...
      {showCropper ? (
        <div className="crop-contain">
          ...
          />
          <button className="set-image" onClick={($event) => getData($event)}>
            Done
          </button>
        </div>
      ) : (
            ...
            <animated.div style={height} className="room-form">
              <animated.div style={show} className="create-room-head">
                <div
                  ...
                  className="room-image-icon"
                  style={{
                    backgroundImage: ` url(${
                      ended
                        ? ended
                        : `${api}/upload/room/default.png?token=${token}`
                    })`,
                ...
                <div className="room-title-icon">
                  ...
                  <input
                    ...
                    type="text"
                    placeholder="Pigeon room"
                  />
                  {formik.errors.title ? (
                    <animated.div style={spring_error} className="field_error">
                      {formik.errors.title}
                    </animated.div>
                  ) : (
                    ""
              ...
              <animated.div style={show} className="create-room-body">
                ...
                <textarea
                  ...
                  maxLength="150"
                  placeholder="Welcome to pigeon room"
                ></textarea>
              ...
              <animated.div style={show} className="create-room-footer">
                  ...
                  {membersView.map((e, i) => {
                    return (
                      <animated.div key={i} className="members-list">
                        <img
                          src={`${api}/upload/user/${e.contact_id.img}?token=${token}`}
                          alt={`${e.contact_id.username} img`}
                        />
                      </animated.div>
                    );
                  })}
                ...
                <p>
                  {members.length > 6
                    ? `And others ${members.length - 6} members`
                    : ""}{" "}
                </p>
              </animated.div>
            </animated.div>
            <div className="contact-list">
              <animated.div style={width} className="">
                ...
                <animated.ul style={show}>
                        ...
                        >
                          <div style={{ background: "transparent" }}>
                            ...
                            >
                              ...
                              <small>
                                {e.contact_id.online ? "Online" : "Offline"}
                              </small>
                            </div>
                          </div>
                          <i
                            onClick={() => addMember(e, i)}
                            className={`bi bi-${
                              e.toggle ? "dash" : "plus"
                            }-square`}
                          ></i>
                  ...
                  {contacts.filter((e) => (e.contact_id ? e : null)).length <=
                  0 ? (
                    <p>
                      Add some contacts in <i className="bi bi-person-plus"></i>
                    </p>
                  ) : (
                    ""
                  )}
...
const SingleUser = ({ userData, token }) => {
  const { user, setAlert } = useContext(userContext);
  const [pressed, setPressed] = useState(false);

  const handleRequest = (username) => {
    sendContact(user.username, "", username, user.img, token, (res) => {
      if (!res.ok) {
        setAlert({ show: true, text: "error" });
        return;
      } else {
  ...
  return (
    ...
    //     {userData.img ? (
    //       <img
    //         src={`${api}/upload/user/${userData.img}?token=${token}`}
    //         alt=""
    //       />
    //     ) : (
    //       ""
    ...
    <Flex justifyContent={"space-between"} my={5}>
        ...
        <Avatar
          ...
          src={
            userData.img == "default.png"
              ? false
              : `${api}/upload/user/${userData.img}?token=${token}`
          }
        >
          {userData.online? (
            ...
            <AvatarBadge boxSize='1em' bg='black' />
          )}
      ...
      <IconButton
        aria-label="Request contact"
        icon={pressed ? <CheckIcon /> : <AddIcon />}
        onClick={() => {
          setPressed(true);
          handleRequest(userData.username);
        }}
      />
    </Flex>
  );
...
const Result = ({ username, state }) => {
  ...
  const circleLoad = useSpring({
    to: { rotate: 190, borderRadius: 5 },
    from: { rotate: 0, borderRadius: 10 },
    loop: true,
  });
  const opacDeep = useSpring({
    to: { opacity: 1 },
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoaded(false);
      let res = await fetch(`${api}/user/search/${username}`, {
        withCredentials: true,
    ...
    <>
      <Box w="100%" px="3">
        {loaded ? (
          userData.length > 0 ? (
            userData.map((e) => {
              return <SingleUser userData={e} token={token} />;
            })
          ) : (
            <NotDefined item="Contact" />
          )
        ) : userData.length > 0 ? (
          userData.map((e) => {
            return <SingleUser userData={e} token={token} />;
...
const AddMember = ({ chats }) => {
  const { contacts, setContacts, selected, setFocus, refresh_rooms } =
    useContext(roomsContext);
  const [members, setMembers] = useState([]);
  ...
  const addMember = (e, i) => {
    ...
    if (respo > -1) {
      setAlert({
        type: "info",
        text: `Member / already in room`,
        resalt: `${contactFiltered[i].contact_id.username}`,
        show: true,
      });
      return;
    }
  ...
  const joinMembers = async () => {
    if (members.length > 0) {
      ...
      setAlert({
        type: "info",
        text: "Request to join / sended",
        resalt: selected.name,
      });
    } else {
            ...
            {selected.members.map((e, i) => {
              return (
                <animated.div key={i} style={show} className="current-user">
                  <img
                    src={`${api}/upload/user/${e.img}?token=${token}`}
                    alt={`${e.username} img`}
                  />
                  <animated.div>
                    <h4>{e.username}</h4>
                ...
                .map((e, i) => {
                  return (
                    ...
                    >
                      ...
                      <i
                        onClick={() => addMember(e, i)}
                        className={`bi bi-${e.toggle ? "dash" : "plus"}-square`}
                      ></i>
                    </li>
                  );
                })}
